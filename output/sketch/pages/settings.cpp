#include "pages.h"
#include <string>

std::string SettingsPage::setHeader()
{
  std::string result = "";
  result += "<head>";
  //result += "<meta http-equiv='refresh' content='10'/>";
  result += "<title>ESP8266 Demo</title>";
  result += "<style> body { background-color: #fffff; font-family: Arial, Helvetica, Sans-Serif; Color: #000088; }</style>";
  result += "<style> #logo { height: 100px; display: block; margin: auto; } </style>";
  result += "<style> #mycontainer { width: 50%; } </style>";
  result += "<link rel=\"stylesheet\" type=\"text/css\" href=\"https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/css/bootstrap.min.css\">";
  result += "<script src=\"https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/js/bootstrap.bundle.min.js\"></script>";
  result += "</head>";
  return result;
}

std::string SettingsPage::setBody()
{
  std::string result = "";
  result += "<body>";
  result += "<div class=\"container\">";
  result += "<img id=\"logo\" src=\"data:image/svg+xml;base64,\" alt=\"none\"/>";
  result += "</div>";

  result += "<h1 class=\"text-center\">Settings page</h1>";
  result += "<div id=\"mycontainer\"  class=\"container\">";

  result += "<form action='/submit' method='POST'>";

  result += "<div class=\"form-group\">";
  result += "<label for=\"senzorid\">SenzorID</label>";
  result += "<input type=\"text\" class=\"form-control\" id=\"senzorid\" name=\"senzorid\" placeholder=\"Enter Senzor ID\">";
  result += "</div>";

  result += "<div class=\"form-group\">";
  result += "<label for=\"wifissid\">Wifi SSID</label>";
  result += "<input type=\"text\" class=\"form-control\" id=\"wifissid\" name=\"wifissid\"  placeholder=\"Enter Wifi SSID\">";
  result += "</div>";

  result += "<div class=\"form-group\">";
  result += "<label for=\"wifipsswd\">Wifi Password</label>";
  result += "<input type=\"password\" class=\"form-control\" id=\"wifipsswd\" name=\"wifipsswd\"  placeholder=\"Enter Wifi password\">";
  result += "</div>";

  result += "<div class=\"form-group\">";
  result += "<label for=\"mqtturl\">MQTT Url : Port</label>";
  result += "<input type=\"text\" class=\"form-control\" id=\"mqtturl\" name=\"mqtturl\"  placeholder=\"Enter MQTT Url : Port\">";
  result += "</div>";

  result += "<div class=\"form-group text-center\">";
  result += "<button type=\"submit\" class=\"btn btn-success\">Save settings</button>";
  result += "</div>";

  result += "</form>";

  result += "</div>";
  result += "</body>";

  return result;
}

std::string SettingsPage::getHTML()
{
  std::string page = "<html>";
  page += setHeader();
  page += setBody();
  page += "</html>";
  return page;
}
